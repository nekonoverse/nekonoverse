"""Fixtures and helpers for Pleroma cross-platform federation tests."""
...
PLEROMA_URL = os.environ.get("PLEROMA_URL", "https://pleroma")
...
class PleromaClient:
    """Helper to interact with Pleroma instance via Mastodon-compatible API."""
    ...
    def register(self, username: str, email: str, password: str):
        """Register a new user via Mastodon-compatible API."""
        ...
        resp.raise_for_status()
        data = resp.json()
        # Registration returns an access token directly
...
def wait_for_instances():
    ...
    wait_for_health(PLEROMA_URL, "/api/v1/instance")
...
@pytest.fixture(scope="session")
def pleroma() -> PleromaClient:
    return PleromaClient(PLEROMA_URL, PLEROMA_DOMAIN)
...
@pytest.fixture(scope="session")
def bob(pleroma: PleromaClient) -> dict:
    """Register bob on Pleroma."""
    data = pleroma.register("bob", "bob@example.com", "Password1234!")
    return data
