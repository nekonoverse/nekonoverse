...
def wait_for_health(url: str, path: str, timeout: int = 180, method: str = "GET"):
...
def poll_until(predicate, *, timeout: int = 60, interval: float = 2.0, desc: str = ""):
...
class NekoClient:
    ...
    def favourite(self, note_id: str):
        resp = self.http.post(f"/api/v1/statuses/{note_id}/favourite")
        resp.raise_for_status()
        return resp.json()
...
class MastodonClient:
    ...
    def register(self, username: str, email: str, password: str):
        """Register a new user via Mastodon API."""
        ...
        # Register user
        resp = self.http.post("/api/v1/accounts", json={
            "username": username,
            "email": email,
            "password": password,
            "agreement": True,
    ...
    def create_status(self, content: str, visibility: str = "public",
                      spoiler_text: str | None = None, in_reply_to_id: str | None = None):
        body = {"status": content, "visibility": visibility}
        if spoiler_text:
            body["spoiler_text"] = spoiler_text
        if in_reply_to_id:
            body["in_reply_to_id"] = in_reply_to_id
        resp = self.http.post("/api/v1/statuses", json=body, headers=self._headers())
        resp.raise_for_status()
        return resp.json()

    def delete_status(self, status_id: str):
        resp = self.http.delete(f"/api/v1/statuses/{status_id}", headers=self._headers())
        resp.raise_for_status()
    ...
    def get_context(self, status_id: str):
        resp = self.http.get(f"/api/v1/statuses/{status_id}/context", headers=self._headers())
        resp.raise_for_status()
        return resp.json()

    def favourite(self, status_id: str):
        resp = self.http.post(f"/api/v1/statuses/{status_id}/favourite", headers=self._headers())
        resp.raise_for_status()
        return resp.json()

    def reblog(self, status_id: str):
        resp = self.http.post(f"/api/v1/statuses/{status_id}/reblog", headers=self._headers())
        resp.raise_for_status()
        return resp.json()
    ...
    def timeline_public(self, local: bool = False, limit: int = 20):
        ...
        if local:
            params["local"] = "true"
        resp = self.http.get(
            "/api/v1/timelines/public", params=params, headers=self._headers()
        )
        resp.raise_for_status()
        return resp.json()
    ...
    def search_accounts(self, q: str, resolve: bool = False):
        ...
        resp = self.http.get(
            "/api/v1/accounts/search", params=params, headers=self._headers()
        )
        resp.raise_for_status()
        return resp.json()
...
def bob(mastodon: MastodonClient) -> dict:
    """Register bob on Mastodon."""
    data = mastodon.register("bob", "bob@example.com", "Password1234!")
    return data
