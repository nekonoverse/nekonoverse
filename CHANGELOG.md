...
セキュリティ監査で見つかった複数の不備をまとめて修正した。Critical (認証バイパス・RCE・SQLi) 相当のものはなかったが、可視性・認可・連合・SSRF・スコープ管理にまたがる修正を6本のPRに分けて行った。

- **ノートの可視性チェックを全経路で統一** — ピン留めノートの AP `/featured` コレクション、リストタイムライン、リアクション/ブックマーク/引用/返信/投票 API などで公開範囲 (followers/direct) の検証が一部欠けており、本来閲覧できないはずのフォロワー限定・DM ノートの内容が渡り得た。全経路で可視性検証を通すよう統一した (#1124)
- **連合で受信した Activity の操作主体を署名者に限定** — Undo (Follow/Like/Announce/Block の取り消し) や Create(Note) の投稿者・投票者の判定が HTTP Signature の署名者ではなく activity 内の任意フィールドを信頼しており、他インスタンスの参加者が別アクター名義で操作できる余地があった。署名検証済みの actor に固定し、HTTP Signature の署名対象ヘッダーも必須化した (#1125)
- **外部リクエストの SSRF 対策を強化** — アクター/ノート取得やメディアプロキシでリダイレクト追従時に安全性を再検証していなかったほか、DNS 解決と実際の接続が分離しておりリバインディングの余地があった。リダイレクト毎の検証と、検証済み IP へ直接接続するトランスポートを導入し、メディアプロキシはストリーミング読み込みに変更してサイズ上限超過時に打ち切るようにした (#1126)
- **リバースプロキシ経由でのクライアント IP 取得を修正** — 本番構成 (nginx → uvicorn UDS) でクライアント IP がアプリに伝わっておらず、ログイン試行回数制限等が実質インスタンス全体で共有される1バケットになっていた。nginx で信頼できる前段プロキシからの `X-Forwarded-For` のみを real_ip として復元し、uvicorn 側でも受け取れるよう設定した (#1127)
- **認証・管理 API・連合受信の堅牢化** — パスワードリセット時に既存セッション/OAuth トークンが失効しない、TOTP のブルートフォース制限が保留トークン単位でリセットできる、削除済みアカウントへのログインが 500 になる、管理 API を OAuth トークンで呼ぶ際にスコープを検証していない、`require_signin_to_view` がフォロー一覧や検索で無視される等、複数の不備をまとめて修正した (#1128)
- **OAuth の認可スコープがアプリ登録時の宣言を超えて要求できた** — `/oauth/authorize` や `/oauth/token` の `scope` パラメータをアプリ登録時の宣言スコープと照合しておらず、ユーザーの同意さえ得られれば宣言していない `admin:write` 等のスコープも取得できた。登録スコープの範囲内であることを検証し、`admin:*` はどのアプリにも一切付与しないようにした (#1129)
- **ブロックが宛先ユーザー以外にも波及していた** — 受信 Activity のブロック判定が「送信者を誰か1人でもブロックしているか」のみを見ており、無関係な他ユーザー宛の Follow/Like/Announce までインスタンス全体で届かなくなっていた。宛先ローカルユーザー単位の判定に修正した (#1129)
