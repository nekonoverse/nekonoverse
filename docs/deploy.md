...
`face-detect/` は MTCNN (PyTorch) による顔検出マイクロサービスで、アップロードされた画像から顔の位置を検出し、フォーカルポイントを自動設定する。GPU マシンに分離することで、メインサーバーの負荷を分散できる。
...
# GPU マシンで起動
cd /opt/face-detect
pip install -r requirements.txt   # torch, facenet-pytorch, fastapi, uvicorn, pillow
...
# → {"status":"ok","device":"cuda"}  (GPU利用時)
# → {"status":"ok","device":"cpu"}   (CPUフォールバック時)
...
| エンドポイント | メソッド | 説明 |
|---------------|---------|------|
| `/health` | GET | ヘルスチェック。`device` フィールドで CUDA 利用状況を返す |
| `/object-detection` | POST | 顔検出。HuggingFace Inference API 互換 |
...
バックエンドはこのバウンディングボックスの中心座標を正規化 `[-1, 1]` のフォーカルポイントに変換して `drive_files.focal_x` / `focal_y` に保存する。
