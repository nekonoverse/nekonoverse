...
`face-detect/` は顔検出マイクロサービスで、アップロードされた画像から顔の位置を検出し、フォーカルポイントを自動設定する。GPU マシンに分離することで、メインサーバーの負荷を分散できる。

検出は2段階で行われる:

1. **アニメ顔検出** (`lbpcascade_animeface`, OpenCV) — 高速、CPU のみで動作
2. **実写顔検出** (MTCNN, PyTorch) — アニメ顔が見つからない場合のフォールバック
...
# GPU マシンで起動
cd /opt/face-detect
pip install -r requirements.txt   # torch, facenet-pytorch, opencv, fastapi, uvicorn, pillow
...
# → {"status":"ok","device":"cuda","models":{"anime":true,"real":true}}
