...
## 外向き HTTP の IP 秘匿 (Cloudflare WARP)

フェデレーション配送やリモートメディア取得時の送信元 IP を、Cloudflare WARP 経由の SOCKS5 プロキシで秘匿する構成。バックエンドは `HTTPS_PROXY` 環境変数だけで透過的に WARP を経由する。

### 仕組み

- `backend/app/utils/http_client.py` の `make_async_client()` が `settings.https_proxy` を自動注入
- 内部サービス (face-detect, neko-vision, media-proxy-rs, video-thumb, summary-proxy, neko-search) はコード側で `proxy=None` に固定されており、WARP を経由しない
- 出口 IP は Cloudflare の ASN になる

### セットアップ

1. **WARP サービスのコメント解除** — `docker-compose.prod.yml` の `warp:` セクションを有効化
2. **環境変数の設定** — `.env` に以下を追加:
   ```bash
   HTTP_PROXY=socks5://warp:1080
   HTTPS_PROXY=socks5://warp:1080
   NO_PROXY=nekono3s,valkey,postgresql,localhost
   ```
3. **起動** — `docker compose -f docker-compose.prod.yml up -d warp` で WARP コンテナを先に起動、ヘルスチェック通過を確認してから `app` / `worker` を再起動
4. **疎通確認** — `docker compose exec app python -c "import httpx; r = httpx.get('https://www.cloudflare.com/cdn-cgi/trace'); print(r.text)"` で `warp=on` が返れば成功

### 注意点

!!! warning "WARP 経由の配送ブロック"
    出口 IP が Cloudflare ASN になるため、一部 fediverse 鯖 (特に VPN/CDN IP をブロックしている Mastodon 管理者) に配送が届かない可能性がある。有効化後は配送成功率を監視し、許容できないほど下がる場合は無効化すること。

!!! note "プラン選択"
    `caomingjun/warp` イメージは WARP 無料プランで動作するが、**無料プランのサーバ用途はグレー**。本番運用では Zero Trust Teams プランを契約し `WARP_LICENSE_KEY` を設定することを推奨。
...
| ファイル | 内容 | 形式 |
|---------|------|------|
| `actor.json` | プロフィール情報 | ActivityPub Actor |
| `outbox.json` | 全投稿 | ActivityPub OrderedCollection |
| `following_accounts.csv` | フォローリスト | Mastodon互換CSV |
| `followers_accounts.csv` | フォロワーリスト | CSV |
| `bookmarks.csv` | ブックマーク | ノートURL一覧 |
| `blocked_accounts.csv` | ブロックリスト | CSV |
| `muted_accounts.csv` | ミュートリスト | CSV |
| `media/` | アップロードしたメディア | 元ファイル |
...
| メソッド | パス | 説明 |
|---------|------|------|
| `POST` | `/api/v1/export` | エクスポート開始（24時間に1回） |
| `GET` | `/api/v1/export` | 最新エクスポートのステータス取得 |
| `GET` | `/api/v1/export/{id}/download` | ZIPダウンロード（本人のみ） |

!!! note "FRONTEND_URL の設定"
